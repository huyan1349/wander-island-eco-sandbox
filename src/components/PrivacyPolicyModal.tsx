...
const PRIVACY_CONTENT = [
  {
    ...
    items: [
      {
        label: '政策效力与适用范围',
        desc: '《流浪岛隐私政策》（以下简称「本政策」）适用于由「启源开物」团队（以下简称「我们」）开发并运营的「流浪岛」生态沙盒游戏（以下简称「本游戏」）的所有相关服务，包括但不限于：游戏客户端（网页端）、游戏服务器、社交功能、AI 对话功能及相关网站页面。\n\n本政策适用于我们通过上述服务收集和处理您个人信息的所有场景。如果您使用第三方账号登录或通过第三方平台访问本游戏，该第三方的隐私政策也将适用于您在其平台上的行为，我们建议您同时仔细阅读该第三方的隐私政策。',
      },
      {
        label: '未成年人特别提示',
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: '4.2 数据存储期限',
        desc: '我们仅在为您提供服务所必需的期限内保留您的个人信息，具体如下：\n\n• 账号信息：在您使用服务期间持续保留，直至您注销账号\n• 游戏存档数据：在您使用服务期间持续保留，直至您删除或清除\n• 聊天记录：在您使用服务期间持续保留，用于对话连贯性\n• 操作日志：保留 90 天，用于安全审计和故障排查\n• CDN 日志：由 Cloudflare 保留，详见 Cloudflare 隐私政策\n\n当您注销账号或要求删除数据时，我们将在 30 个工作日内删除或匿名化处理您的个人信息。超出上述保留期限后，我们将对您的个人信息进行删除或匿名化处理。',
      },
      {
        label: '4.3 安全技术措施',
        desc: '我们采用以下技术措施保护您的个人信息安全：\n\n• 传输加密：所有客户端与服务器之间的通信均使用 HTTPS/TLS 1.2+ 加密\n• 密码保护：使用 bcrypt 算法对密码进行单向哈希存储，无法还原明文\n• 访问控制：实施基于角色的访问控制，仅授权人员可访问用户数据\n• 数据隔离：不同用户的数据在逻辑上隔离存储\n• 输入验证：对所有用户输入进行验证和过滤，防止注入攻击\n• 安全审计：记录关键操作的审计日志\n• 漏洞管理：定期进行安全评估和漏洞修复',
      },
      {
        label: '4.4 安全管理制度',
        desc: '我们建立了以下安全管理制度：\n\n• 人员管理：对接触个人信息的人员进行安全培训和考核\n• 制度规范：制定数据安全管理规范和操作流程\n• 应急响应：建立个人信息安全事件应急预案\n• 合规审查：定期审查数据处理活动的合规性',
      },
      {
        label: '4.5 安全事件应急',
        desc: '如不幸发生个人信息安全事件，我们将按照法律法规的要求，及时向您告知安全事件的基本情况和可能的影响、我们已采取或将要采取的处置措施、您可自主防范和降低风险的建议，并对您进行补偿/救济。\n\n我们将以推送通知、邮件、信箱公告等方式告知您，难以逐一告知时我们会采取公告方式告知。同时，我们还将按照监管部门要求，主动上报个人信息安全事件的处置情况。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: 'Cloudflare 可能设置以下 Cookie：\n\n• __cfduid：用于识别受信任的 Web 流量，不包含个人身份信息\n• __cf_bm：用于 Bot 管理，不包含个人身份信息\n• cf_clearance：用于验证已通过安全检查的访问者\n\n这些 Cookie 是 Cloudflare 安全防护功能的一部分，不包含您的个人身份信息。您可以通过浏览器设置管理或删除这些 Cookie，但删除后可能影响网站的正常访问。',
      },
      {
        label: '7.3 会话存储（sessionStorage）',
        desc: '我们使用浏览器的 sessionStorage 技术在您的设备本地临时存储会话级别的信息。这些信息在您关闭浏览器标签页后自动清除，不会持久化存储。',
      },
      {
        label: '7.4 IndexedDB',
        desc: '我们可能使用浏览器的 IndexedDB 技术在您的设备本地缓存游戏资源文件（如图片、音频），以提升加载速度和离线体验。IndexedDB 中不存储您的个人信息。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '8.1 境内存储',
        desc: '您的个人信息存储在中华人民共和国境内的阿里云服务器上，我们不会主动将您的个人信息传输至境外。',
      },
      {
        label: '8.2 CDN 全球加速',
      ...
      },
      {
        ...
        desc: '当您使用「辞」聊天功能时，您的消息内容会被发送至 DeepSeek API。DeepSeek 的服务器可能位于中国境内或境外。我们已与 DeepSeek 签署数据处理协议，要求其按照不低于本政策的安全标准处理您的信息。您选择使用「辞」聊天功能即视为同意您的消息内容可能被传输至 DeepSeek 的服务器进行处理。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '联系方式',
        desc: '如果您对本隐私政策有任何疑问、建议，或需要行使您的个人信息权利（查阅、更正、删除、撤回同意、注销账号、获取副本等），请通过以下方式联系我们：\n\n• 电子邮件：huyanxius@gmail.com\n• 游戏内信箱：发送邮件给「辞」\n\n我们将在 15 个工作日内回复您的请求。',
      },
      {
        label: '争议解决',
    ...
    <div className="hand-drawn-panel w-[680px] max-w-[94vw] max-h-[88vh] p-0 flex flex-col animate-slide-up ring-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div className="mb-6 text-center">
          <p className="text-slate-400 text-[10px]">生效日期：2026 年 6 月 16 日 · 最近更新：2026 年 6 月 16 日 · 版本：v1.0</p>
          <p className="text-sm text-slate-600 mt-3 leading-relaxed">
            欢迎来到「流浪岛」。我们深知个人信息对您的重要性，并将按照法律法规的规定，保护您的个人信息及隐私安全。我们依据《中华人民共和国个人信息保护法》《中华人民共和国网络安全法》《中华人民共和国数据安全法》等法律法规，制定本隐私政策，以帮助您了解我们如何收集、使用、存储和保护您的信息。请您在使用我们的服务前，仔细阅读并充分理解本政策的全部内容。
          </p>
        ...
        <div className="mt-6 pt-4 border-t border-slate-200/50 text-center">
          <p className="text-slate-400 text-[10px]">流浪岛 · 流浪岛生态沙盒 · 隐私政策 v1.0 · 启源开物</p>
        </div>
      </div>
